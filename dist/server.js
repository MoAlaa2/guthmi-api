...
const translateError = (errorKey, lang = 'ar') => {
    const errors = {
...
class MessageQueue {
    ...
    isProcessing = false;
    emergencyStop = false;
    rateLimitDelay = 100; // ms between messages (synced from settings_protection.base_delay_ms)
    campaignThrottleDelay = 1000; // ms between campaign messages (synced from settings_protection.campaign_delay_ms)
    dailyMessageCount = 0;
    dailyMessageCap = 10000; // Configurable (synced from settings_protection.max_daily_messages)
    lastResetDate = new Date().toDateString();
    campaignThrottles = new Map(); // campaignId -> last send timestamp
    dbInitialized = false;
    constructor() {
    ...
    async initializeFromDB() {
        try {
            ...
            const countRes = await pool.query("SELECT value FROM app_state WHERE key = 'dailyMessageCount'");
    ...
    async saveEmergencyStopToDB() {
        try {
            await pool.query(`INSERT INTO app_state (key, value, updated_at) 
         VALUES ('emergencyStop', $1, NOW())
         ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [JSON.stringify({ enabled: this.emergencyStop })]);
    ...
    async saveDailyCountToDB() {
        try {
            await pool.query(`INSERT INTO app_state (key, value, updated_at)
         VALUES ('dailyMessageCount', $1, NOW())
         ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [JSON.stringify({ count: this.dailyMessageCount, date: this.lastResetDate })]);
        }
        catch (err) {
            console.error('❌ Failed to save dailyCount:', err);
        }
    }
    enqueue(message, campaignId) {
        return new Promise((resolve, reject) => {
            ...
            if (this.dailyMessageCount >= this.dailyMessageCap) {
                reject(new Error('Daily message cap reached'));
                return;
            }
    ...
    async process() {
        if (this.isProcessing || this.queue.length === 0 || this.emergencyStop)
            return;
        ...
        if (item) {
            try {
                ...
                if (item.message.type === 'text')
                    payload.text = item.message.content;
                else if (item.message.type === 'template')
                    payload.template = item.message.content;
                else if (item.message.type === 'interactive')
                    payload.interactive = item.message.content;
                else if (item.message.type === 'image')
                    payload.image = item.message.content;
...
setInterval(async () => {
    try {
        const { rows } = await pool.query(`SELECT * FROM scheduled_bot_actions 
       ...
       LIMIT 10`);
        for (const action of rows) {
            try {
                ...
                const convRes = await pool.query('SELECT contact_number FROM conversations WHERE id = $1', [action.conversation_id]);
                if (convRes.rowCount === 0) {
                    console.warn(`Conversation ${action.conversation_id} not found`);
                    await pool.query(`UPDATE scheduled_bot_actions SET status = 'failed' WHERE id = $1`, [action.id]);
                    continue;
                }
                ...
                const node = flow.nodes?.find((n) => n.id === action.node_id);
                if (node) {
                    ...
                    await pool.query(`UPDATE scheduled_bot_actions SET status = 'executed' WHERE id = $1`, [action.id]);
...
// --- CHATBOT ENGINE ---
const processBotLogic = async (conversation, messageContent, from) => {
    try {
        ...
        if (!currentFlowId) {
            ...
            for (const flow of flows) {
                if (flow.triggers && flow.triggers.includes(messageContent.toLowerCase().trim())) {
                    ...
                    let startNode = flow.nodes.find((n) => n.type === 'start');
                    if (!startNode) {
                        startNode = flow.nodes.find((n) => !flow.edges.some((e) => e.target === n.id));
                    }
        ...
        if (currentFlowId) {
            ...
            const currentNode = flow.nodes.find((n) => n.id === botState.currentNodeId);
            if (currentNode) {
                ...
                if ((currentNode.type === 'question' || currentNode.type === 'interactive') && currentNode.data.options) {
                    const selectedOption = currentNode.data.options.find((opt) => opt.label && opt.label.toLowerCase().trim() === messageContent.toLowerCase().trim());
                    if (selectedOption) {
                        // Find edge with matching sourceHandle (option id)
                        edge = flow.edges.find((e) => e.source === currentNode.id && e.sourceHandle === selectedOption.id);
                    }
                }
...
const executeFlowNode = async (convId, to, node, flow, botState = {}) => {
    try {
        // 🛡️ ANTI-LOOP GUARD: Prevent infinite loops
        const MAX_STEPS = 20;
        if (!botState.stepCount)
        ...
        if (botState.stepCount > MAX_STEPS) {
            ...
            await pool.query(`UPDATE conversations SET active_flow_id = NULL, bot_state = '{}'::jsonb WHERE id = $1`, [convId]);
            // Send error message to user
            await outboundQueue.enqueue({
                to,
                type: 'text',
        ...
        if (node.type === 'message' || node.type === 'text') {
            ...
            await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status)
         VALUES ($1, 'outbound', 'text', $2, 'sent')`, [convId, node.data.content || node.data.message]);
        }
            ...
            if (buttons.length > 0 && buttons.length <= 3) {
                ...
                await outboundQueue.enqueue({
                    ...
                    content: {
                        ...
                        action: {
                            buttons: buttons.slice(0, 3).map((opt, idx) => ({
                                type: 'reply',
                                reply: { id: `opt_${idx}`, title: opt.label || opt }
                            }))
                        }
                    },
                    priority: 1
                ...
                await outboundQueue.enqueue({
                    to,
                    type: 'text',
                    content: { body: (node.data.content || '') + "\n\n" + buttons.map((b, i) => `${i + 1}. ${b.label || b}`).join("\n") },
                    priority: 1
                });
            }
            await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status)
         VALUES ($1, 'outbound', 'interactive', $2, 'sent')`, [convId, node.data.content || node.data.message]);
            ...
            if (imageUrl) {
                ...
                await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status)
           VALUES ($1, 'outbound', 'image', $2, 'sent')`, [convId, node.data.caption || '[Image]']);
            }
        }
            ...
            if (nextEdge) {
                ...
                if (nextNode) {
                    ...
                    await pool.query(`INSERT INTO scheduled_bot_actions (conversation_id, node_id, flow_id, execute_at, bot_state)
             VALUES ($1, $2, $3, $4, $5)`, [convId, nextNode.id, flow.id, executeAt, JSON.stringify(botState)]);
...
app.post('/api/webhooks/whatsapp', verifySignature, webhookLimiter, async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, metadata) VALUES (NULL, 'WEBHOOK_RAW', $1)`, [req.body]);
        ...
        for (const entry of entries) {
            ...
            for (const change of changes) {
                ...
                if (value?.messages?.length) {
                    for (const msg of value.messages) {
                        ...
                        const existingConv = await pool.query('SELECT id, status FROM conversations WHERE contact_number = $1', [from]);
                        const wasClosedAndReopening = existingConv.rows[0]?.status === 'closed';
                        const convRes = await pool.query(`INSERT INTO conversations (contact_number, status, last_message, last_message_at, last_customer_message_at, window_expires_at, unread_count)
           ...
           RETURNING id`, [from, text, timestamp, new Date(timestamp.getTime() + 24 * 60 * 60 * 1000)]);
                        const conversationId = convRes.rows[0].id;
                        // Log reopen event if was closed
                        if (wasClosedAndReopening) {
                            await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, metadata)
             VALUES ($1, 'conversation_reopened', $2)`, [conversationId, { reason: 'customer_message', from }]);
                            console.log(`🔄 Conversation ${conversationId} reopened by customer message`);
                        }
                        await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status, meta_id, media_url, media_mime_type, media_sha256, media_file_size, created_at)
           VALUES ($1, 'inbound', $2, $3, 'delivered', $4, $5, $6, $7, $8, $9)`, [conversationId, msg.type || 'text', text, messageId, mediaUrl, mediaMimeType, mediaSha256, mediaFileSize, timestamp]);
                        await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, metadata)
           VALUES ($1, 'inbound_received', $2)`, [conversationId, { from, msg }]);
                ...
                if (value?.statuses?.length) {
                    for (const st of value.statuses) {
                        ...
                        await pool.query(`UPDATE messages SET status = $1, updated_at = NOW() WHERE meta_id = $2`, [newStatus, wamid]);
                        const convRow = await pool.query(`SELECT conversation_id FROM messages WHERE meta_id = $1 LIMIT 1`, [wamid]);
                        const conversationId = convRow.rows[0]?.conversation_id || null;
                        await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, metadata)
           VALUES ($1, $2, $3)`, [conversationId, newStatus, st]);
                    }
                }
            } // End of changes loop
        } // End of entries loop
...
app.post('/api/conversations/:id/assign', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await pool.query('INSERT INTO conversation_assignments (conversation_id, agent_id, assigned_by) VALUES ($1, $2, $3)', [id, agentId, assignedBy]);
        await pool.query('INSERT INTO conversation_events (conversation_id, event_type, metadata) VALUES ($1, $2, $3)', [id, 'agent_assigned', JSON.stringify({ agentId, assignedBy })]);
    ...
    try {
        ...
        if (assignedAgentId !== undefined) {
            ...
            if ((convRes.rowCount ?? 0) > 0) {
                ...
                if (oldAgentId) {
                    await pool.query(`UPDATE conversation_assignments SET status = 'inactive', unassigned_at = NOW()
             WHERE conversation_id = $1 AND agent_id = $2 AND status = 'active'`, [req.params.id, oldAgentId]);
                }
                // Assign new agent if provided
                if (assignedAgentId) {
                    await pool.query(`INSERT INTO conversation_assignments (conversation_id, agent_id, assigned_by)
             VALUES ($1, $2, $3)`, [req.params.id, assignedAgentId, userId]);
                    await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, user_id, user_name, metadata)
             VALUES ($1, 'agent_assigned', $2, $3, $4)`, [req.params.id, userId, userName, JSON.stringify({ agent_id: assignedAgentId })]);
                }
        ...
        if (tags !== undefined) {
            ...
            await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, user_id, user_name, metadata)
         VALUES ($1, 'tags_updated', $2, $3, $4)`, [req.params.id, userId, userName, JSON.stringify({ tags })]);
        }
        // Handle status change
        if (status) {
            await pool.query('UPDATE conversations SET status = $1 WHERE id = $2', [status, req.params.id]);
            await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, user_id, user_name, metadata)
         VALUES ($1, 'status_changed', $2, $3, $4)`, [req.params.id, userId, userName, JSON.stringify({ new_status: status })]);
        }
        res.json({ success: true });
    }
    catch (err) {
...
app.post('/api/messages', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (convRes.rowCount === 0)
            return res.status(404).json({ error: translateError('CONVERSATION_NOT_FOUND', lang) });
        ...
        const insertRes = await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status)
       VALUES ($1, 'outbound', $2, $3, $4) RETURNING *`, [conversationId, type || 'text', content, type === 'note' ? 'delivered' : 'sent']);
...
app.post('/api/conversations/:id/close', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Log event
        await pool.query(`INSERT INTO conversation_events (conversation_id, event_type, user_id, user_name, metadata)
       VALUES ($1, 'conversation_closed', $2, $3, $4)`, [id, userId, userName, JSON.stringify({ reason: 'manual_close' })]);
        // Send Feedback Template via Queue (using ENV config + language detection)
        try {
            // Detect conversation language from recent messages
            const langRes = await pool.query(`SELECT content FROM messages 
         ...
         ORDER BY created_at DESC LIMIT 5`, [id]);
            ...
            await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status)
         VALUES ($1, 'outbound', 'template', $2, 'sent')`, [id, `Feedback Template: ${FEEDBACK_TEMPLATE_NAME} (${detectedLang})`]);
        }
        catch (templateErr) {
...
app.get('/api/conversations/:id/details', authenticateToken, async (req, res) => {
    try {
        ...
        if (convRows.rowCount === 0)
            return res.status(404).json({ error: 'Not found' });
        const conversation = convRows.rows[0];
        const { rows: events } = await pool.query(`SELECT * FROM conversation_events WHERE conversation_id = $1 ORDER BY created_at DESC LIMIT 50`, [req.params.id]);
        const { rows: assignments } = await pool.query(`SELECT ca.*, u.name as agent_name FROM conversation_assignments ca
       LEFT JOIN users u ON ca.agent_id = u.id
       WHERE ca.conversation_id = $1 ORDER BY ca.assigned_at DESC`, [req.params.id]);
...
app.post('/api/queue/set-daily-cap', authenticateToken, async (req, res) => {
    ...
    const lang = req.headers['accept-language']?.includes('en') ? 'en' : 'ar';
    if (!cap || cap < 0)
        return res.status(400).json({ error: translateError('INVALID_CAP', lang) });
...
app.post('/api/automation/flows', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (exists.length > 0) {
            const { rows } = await pool.query('UPDATE chat_flows SET name=$1, nodes=$2, edges=$3, active=$4, last_modified=NOW() WHERE id=$5 RETURNING *', [flow.name, JSON.stringify(flow.nodes), JSON.stringify(flow.edges), flow.status === 'active', flow.id]);
            res.json(rows[0]);
        }
        else {
            const { rows } = await pool.query('INSERT INTO chat_flows (id, name, nodes, edges, active) VALUES ($1, $2, $3, $4, $5) RETURNING *', [flow.id, flow.name, JSON.stringify(flow.nodes), JSON.stringify(flow.edges), flow.status === 'active']);
            res.json(rows[0]);
        }
    }
    catch (e) {
...
app.post('/api/templates', authenticateToken, async (req, res) => {
    try {
        const { data } = await axios.post(`${BASE_URL}/${WABA_ID}/message_templates`, req.body, { headers: { Authorization: `Bearer ${META_ACCESS_TOKEN}`, 'Content-Type': 'application/json' } });
        res.json(data);
    }
    catch (err) {
...
app.delete('/api/templates/:id', authenticateToken, async (req, res) => {
    try {
        const { data } = await axios.delete(`${BASE_URL}/${WABA_ID}/message_templates`, {
...
app.post('/api/messages/template', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const response = await axios.post(`${BASE_URL}/${PHONE_NUMBER_ID}/messages`, payload, { headers: { Authorization: `Bearer ${META_ACCESS_TOKEN}`, 'Content-Type': 'application/json' } });
        const metaId = response.data.messages?.[0]?.id;
        // Store in database
        const insertRes = await pool.query(`INSERT INTO messages (conversation_id, direction, type, content, status, meta_id)
       VALUES ($1, 'outbound', 'template', $2, 'sent', $3) RETURNING *`, [conversationId, `Template: ${templateName}`, metaId]);
        // Update conversation
        await pool.query(`UPDATE conversations SET last_message = $1, last_message_at = NOW() WHERE id = $2`, [`Template: ${templateName}`, conversationId]);
        res.json({
            success: true,
...
// Contacts API
// =========== AUTO-REPLIES API ===========
// ⚠️ NOTE: Auto-Replies are currently PLACEHOLDERS
// These endpoints return mock/empty data for UI compatibility
// To implement fully: Create auto_replies table and add trigger logic
// Get Auto Replies
app.get('/api/automation/auto-replies', authenticateToken, async (req, res) => {
    try {
        // Placeholder - return empty array for now
        // TODO: Query from auto_replies table when implemented
        res.json([]);
    }
    catch (err) {
...
app.post('/api/automation/auto-replies', authenticateToken, async (req, res) => {
    try {
        ...
        const mockReply = { id: Date.now().toString(), ...req.body, createdAt: new Date() };
        res.json(mockReply);
    }
    catch (err) {
...
app.get('/api/contacts', authenticateToken, async (req, res) => {
    try {
        const { rows } = await pool.query(`
      SELECT DISTINCT contact_number as "phoneNumber", contact_name as "name", tags, 
...
app.post('/api/contacts', authenticateToken, async (req, res) => {
    ...
    const lang = req.headers['accept-language']?.includes('en') ? 'en' : 'ar';
    if (!phone)
        return res.status(400).json({ error: translateError('PHONE_REQUIRED', lang) });
    try {
        const { rows } = await pool.query(`
...
app.post('/api/orders', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      RETURNING *
    `, [orderNumber, conversationId, customerPhone, customerName, JSON.stringify(items || []), subtotal || 0, shippingCost || 0, total || 0, JSON.stringify(shippingAddress), notes, req.user?.id]);
        // Log history
        await pool.query(`
      INSERT INTO order_history (order_id, action, user_id, user_name)
      VALUES ($1, 'created', $2, $3)
    `, [rows[0].id, req.user?.id, req.user?.name || 'System']);
        res.json({
            id: rows[0].id,
...
app.post('/api/tags', authenticateToken, async (req, res) => {
    ...
    const lang = req.headers['accept-language']?.includes('en') ? 'en' : 'ar';
    if (!name)
        return res.status(400).json({ error: translateError('NAME_REQUIRED', lang) });
    try {
        const { rows } = await pool.query(`
...
app.post('/api/internal-notifications', authenticateToken, async (req, res) => {
    ...
    const lang = req.headers['accept-language']?.includes('en') ? 'en' : 'ar';
    if (!title)
        return res.status(400).json({ error: translateError('TITLE_REQUIRED', lang) });
    try {
        const { rows } = await pool.query(`
...
const verifyEndpointGuard = (req, res, next) => {
    if (!ENABLE_VERIFY) {
